...
PROMPT = ("Process the intake for case {case_id} (requester {req}). The raw application is already ingested as "
          ...
          "case_ref; 2) mask_pii with case_ref; 3) assess_aid with the extracted fields, deidentified true "
...
def main():
    ...
    users = {"cw-a": ["aid_officer", f"tenant_{ta}"], "cw-b": ["aid_officer", f"tenant_{tb}"]}
