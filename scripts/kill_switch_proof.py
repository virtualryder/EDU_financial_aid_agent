...
def main():
    ...
    a = ap.parse_args()
    prefix, region = f"fa-{a.env}", a.region
    ...
    make_user(idp, pool, "ks-cw-a", ["aid_officer", f"tenant_{ta}"], pw)
    make_user(idp, pool, "ks-cw-b", ["aid_officer", f"tenant_{tb}"], pw)
    time.sleep(3)
    ...
    direct = lam.invoke(FunctionName=f"{prefix}-intake-fafsa",
