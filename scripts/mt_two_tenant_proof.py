...
Drives the deployed AgentCore gateway as THREE identities and records verbatim results:
  * cw-a   : aid_officer + tenant_pha-a  -> allowed; mask_pii routes to pha-a's OWN store
  * cw-b   : aid_officer + tenant_pha-b  -> allowed; routes to pha-b's OWN store
  * cw-none: aid_officer, NO tenant      -> DENIED at the gateway (require_tenant / interceptor)
...
def main():
    ...
    users = {"cw-a": ["aid_officer", f"tenant_{ta}"],
             "cw-b": ["aid_officer", f"tenant_{tb}"],
             "cw-none": ["aid_officer"]}
