...
# ---- 2026-09-04: the MCP teardown must not mask a governed mid-session stop -------------------------
def _fake_runtime(monkeypatch, agent_call, exit_raises=None):
    """Wire invoke()'s SDK seams: one authorized tool, a controllable Agent __call__, and an MCP client
    whose __exit__ may raise (the streamable-HTTP 'Connection ... was closed' teardown seen live)."""
    class _Tool:
        tool_name = "intake_fafsa"
    ...
    monkeypatch.setattr(agent, "MCPClient", lambda *a, **k: _MCP())
    monkeypatch.setattr(agent, "Agent", _Agent)
    monkeypatch.setattr(agent, "BedrockModel", lambda *a, **k: object())
    monkeypatch.setattr(agent, "_bedrock_session", lambda corr: object())
    monkeypatch.setattr(agent, "_gateway_url", lambda: "https://gw/mcp")
    monkeypatch.setattr(agent, "_session_tenant", lambda tok: "cw-a")


def test_mcp_teardown_does_not_mask_a_midsession_kill(ssm, monkeypatch):
    """A kill-switch stop unwinding the in-flight session can trip the MCP client's teardown
    ('Connection to the MCP server was closed'); that teardown artifact must NOT mask the governed
    mid-session refusal with a 500 (seen live 2026-09-04). The switch is DISENGAGED at entry (so the
    top-of-invoke guard passes) and the stop is proven by the KillSwitchEngaged in the cause chain."""
    def _raise_kill():
        raise agent.KillSwitchEngaged("stop now")
    _fake_runtime(monkeypatch, _raise_kill,
                  exit_raises=RuntimeError("Connection to the MCP server was closed"))
    out = agent.invoke({"access_token": "t.t.t", "case_id": "C-1", "requester": "r"})
    assert out.get("stopped") == "mid-session"
    assert out.get("guardrail_action") == "KILL_SWITCH" and out.get("refused") is True


def test_mcp_teardown_error_does_not_mask_a_successful_result(ssm, monkeypatch):
    """The same teardown on a NORMAL run is a cosmetic artifact: the successful result stands."""
    _fake_runtime(monkeypatch, lambda: "signoff requested",
                  exit_raises=RuntimeError("Connection to the MCP server was closed"))
    out = agent.invoke({"access_token": "t.t.t", "case_id": "C-1", "requester": "r"})
    assert out.get("result") == "signoff requested" and out.get("tenant") == "cw-a"
    assert "stopped" not in out


def test_unrelated_teardown_error_still_raises(ssm, monkeypatch):
    """A teardown error that is NOT containment on a run that produced no outcome must surface, not be
    swallowed — fail loud, never silently."""
    def _raise_unrelated():
        raise ValueError("model provider 5xx")
    # agent call raises ValueError (not contained, switch disengaged) -> inner except re-raises ->
    # the with-block teardown also raises; either way invoke must not return a bogus success.
    _fake_runtime(monkeypatch, _raise_unrelated,
                  exit_raises=RuntimeError("Connection to the MCP server was closed"))
    with pytest.raises(Exception):
        agent.invoke({"access_token": "t.t.t", "case_id": "C-1", "requester": "r"})
