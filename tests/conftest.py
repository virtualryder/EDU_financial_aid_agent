...
workflow_guards, sanitized, case_store, ingest_case, tenancy, readability) shadows the packaged one
if a name ever collides. `tests/test_core_dependency.py` asserts that every such shadow is a DECLARED
domain override, because a silent shadow would reintroduce exactly the drift the dependency exists to
prevent.

Also set the provenance signing secret ONCE, before any test module is imported, so lookup_coa
(signer) and assess_aid (verifier) use the same key for the P0-3 provenance gate.
"""
import os
