...
MANIFEST = os.path.join(ROOT, "agents", "financial-aid", "manifest.yaml")
POLICIES_DIR = os.path.join(ROOT, "policies")
GW_ARN = "arn:aws:bedrock-agentcore:us-east-1:111122223333:gateway/fa-aid-gw"
...
NINE_CONDITIONS = [
    ("fa_require_entitlement",           'principal.getTag("custom:tools") != ""'),                      # 1 entitlement
    ("fa_mask_before_assess",            "context.input.deidentified == true"),                          # 2 data-class
    ("fa_consent_purpose_before_assess_aid", "context.input has consent && context.input.consent == true"),  # 3 consent
    ("fa_consent_purpose_before_assess_aid", 'context.input.purpose == "aid_determination"'),            # 4 purpose
    ("fa_budget_before_draft_award_notice", "context.input has budget_ok && context.input.budget_ok == true"),  # 5 budget
    ("fa_require_service_window",        "context.input has within_service_window && context.input.within_service_window == true"),  # 6 temporal
    ("fa_amount_cap_assess_aid",         'context.input.cost_of_attendance.lessThanOrEqual(decimal("100000.0"))'),  # 7 quantitative
    ("fa_require_tenant",                'principal.hasTag("custom:tenant")'),                           # 8 tenant
    ("fa_no_self_commit",                'AgentCore::Action::"fa-core___finalize_award"'),                # 9 SoD
]
QUANTITATIVE_NA = False


def test_all_declared_policies_render(policies):
    assert len(policies) == 12, sorted(policies)
...
def test_entitlement_is_zero_default(policies):
    ...
    _mode, stmt = policies["fa_require_entitlement"]
...
def test_every_statement_parses_under_cedar(policies):
    ...
    req = {"principal": 'AgentCore::User::"u"', "action": 'AgentCore::Action::"assess-aid___assess_aid"',
