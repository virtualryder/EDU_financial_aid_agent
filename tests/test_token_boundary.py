"""P0-3 + P0-9 — trusted runtime credential boundary and telemetry redaction.
...
and runtime logs never contain the bearer token (redaction). Pure logic, no AWS."""
import json
import os
import pathlib
import sys
...
MANIFEST = (ROOT / "agents" / "financial-aid" / "manifest.yaml").read_text(encoding="utf-8")
...
def test_no_credential_field_in_any_tool_schema():
    import re
    # every `input:` block key in the manifest; none may be credential-shaped
...
def test_scrub_drops_model_supplied_credentials_everywhere():
    args, dropped = token_boundary.scrub_args(
        {"case": "x", "access_token": "stolen", "Api-Key": "k", "password": "p", "months": 3})
    assert "access_token" not in args and "Api-Key" not in args and "password" not in args
    assert args == {"case": "x", "months": 3}
    assert sorted(dropped) == ["Api-Key", "access_token", "password"]


def test_inject_only_for_signoff_and_overrides_model_token():
    # non-signoff tool: nothing injected even if the model tried to pass a token
    a = token_boundary.prepare_args("assess-aid___assess_aid",
                                    {"annual_income": 1, "access_token": "stolen"}, TOKEN)
    assert "access_token" not in a
    # signoff tool: model-supplied token is DISCARDED; the runtime token is injected
    a = token_boundary.prepare_args("request-signoff___request_signoff",
                                    {"icsr_id": "HOU-1", "access_token": "stolen"}, TOKEN)
    assert a["access_token"] == TOKEN and a["icsr_id"] == "HOU-1"
...
def test_wrap_intercepts_positional_and_keyword_calls():
    ...
    c.call_tool_sync("t2", name="fa-core___draft_award_notice", arguments={"case": "x", "token": "stolen"})
    assert calls[0][1]["access_token"] == TOKEN
    assert "token" not in calls[1][1] and "access_token" not in calls[1][1]


# ── (c) redaction: token never in audit payload or runtime logs ──────────────
...
def test_signoff_audit_payload_contains_no_token(monkeypatch):
    ...
    monkeypatch.setattr(rs.identity, "verify_access_token",
                        lambda tok, require_group=True: ({"username": "housing_specialist",
                                                          "sub": "u-123"}, None))
    ...
    out = rs.handler({"icsr_id": "HOU-1", "access_token": TOKEN}, _Ctx())
...
def test_runtime_logs_token_presence_only():
    src = (ROOT / "lib" / "runtime" / "agent.py").read_text(encoding="utf-8")
    assert "token_present=%s" in src or "token_present" in src   # boolean presence logging
    assert 'log.info("token' not in src.replace("token_present", "")  # never the value
    assert "wrap_mcp_client" in src                              # boundary is wired in


def test_no_role_lookup_by_name_prefix_in_deploy_paths():
    # P0-7 companion gate: no `starts_with(RoleName,...)` discovery in any engine/runtime/connector
    # script. (Deliberately iterates only the script dirs — never the committed .venv tree.)
    bad = []
