...
BACKGROUND
----------
Until 2026-08-03 this repo carried its own copy of the governance controls in `lib/controls/`. Four
copies of the same control plane drifted apart while all four repos' integrity locks recorded the
same tree hash and every CI run was green: this repo HAD the exactly-once `FINAL#` finalization
control and the pharmacovigilance and benefits agents did not, and nothing detected that for weeks.
For a financial aid workflow the absence of that control is a duplicate-award risk: a retried or
replayed finalize writes a second COMMITTED record, i.e. a duplicate aid award or package adjustment
posted for the same student — an overaward the institution must later detect, reverse and report.
...
DOMAIN_OVERRIDES = {
    "mask_pii": "Financial aid masks the student/parent identifiers an ISIR-derived record carries "
                "(name, SSN, address, DOB, phone, bank/routing) and, unlike the packaged control, "
                "persists the masked payload server-side under the deployment-pinned tenant "
                "(SANITIZED_TABLE + tenancy) so the FERPA education-record copy never rides in a "
                "request body.",
    "provenance": "Financial aid binds provenance to the College Scorecard reference feed, which is "
                  "REFERENCE data rather than institutional COA, and carries the GA-2 split signing "
                  "domains this vertical needs: 'deid' (minted by mask_pii) and 'scorecard' (minted "
                  "by lookup_coa), so a compromised masker cannot forge a Scorecard attribution.",
}
...
def test_exactly_once_finalization_is_present_in_the_resolved_core():
    ...
    assert hasattr(finalize_signoff, "_exactly_once_marker"), (
        ...
        "gap re-opening, i.e. a duplicate aid award posted for the same student")
