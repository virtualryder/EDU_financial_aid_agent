...
def _fresh_provenance(monkeypatch, **env):
    for k in ("PROVENANCE_SECRET", "PROVENANCE_SECRET_ARN",
              ...
              "PROVENANCE_SECRET_SCORECARD", "PROVENANCE_SECRET_ARN_SCORECARD"):
...
def test_cross_domain_token_is_a_forgery(monkeypatch):
    ...
    p = _fresh_provenance(monkeypatch, PROVENANCE_SECRET_DEID="deid-key-1",
                          PROVENANCE_SECRET_SCORECARD="scorecard-key-1")
    sc_tok = p.sign("sc-source", {"il50": 83300}, domain="scorecard")
    deid_tok = p.sign("mask-source", {"sanitized_sha256": "abc"}, domain="deid")
    assert sc_tok["authoritative"] and deid_tok["authoritative"]
    # each verifies ONLY in its own domain
    assert p.verify("sc-source", {"il50": 83300}, sc_tok, domain="scorecard") is True
    assert p.verify("mask-source", {"sanitized_sha256": "abc"}, deid_tok, domain="deid") is True
    # cross-domain = forgery
    assert p.verify("sc-source", {"il50": 83300}, sc_tok, domain="deid") is False
    assert p.verify("mask-source", {"sanitized_sha256": "abc"}, deid_tok, domain="scorecard") is False
...
def test_domain_falls_back_to_shared_secret_for_dev(monkeypatch):
    ...
    tok = p.sign("s", {"a": 1}, domain="scorecard")
    assert tok["authoritative"] is True
    assert p.verify("s", {"a": 1}, tok, domain="scorecard") is True
...
def test_configured_domain_key_never_degrades_to_shared(monkeypatch):
    ...
    p = _fresh_provenance(monkeypatch, PROVENANCE_SECRET="shared-dev",
                          PROVENANCE_SECRET_ARN_SCORECARD="arn:aws:secretsmanager:us-east-1:111122223333:secret:hud-key")
...
def test_sanitized_ref_rejected_when_minted_with_hud_key(monkeypatch):
    ...
    forged_tok = p.sign(sanitized.SOURCE, fields, domain="scorecard")
    forged = dict(ref)
...
def test_tokens_stamp_key_version(monkeypatch):
    ...
    p = _fresh_provenance(monkeypatch, PROVENANCE_SECRET_SCORECARD="scorecard-key-1")
    tok = p.sign("s", {"a": 1}, domain="scorecard")
    assert tok["key_version"] == "scorecard:env"
...
def test_scorecard_key_resolves_from_arn_with_demo_fallback(monkeypatch):
    sys.path.insert(0, str(ROOT / "agents" / "financial-aid" / "tools"))
    monkeypatch.delenv("SCORECARD_API_KEY", raising=False)
    monkeypatch.setenv("SCORECARD_API_KEY_ARN", "arn:aws:secretsmanager:us-east-1:111122223333:secret:scorecard")

    class _SM:
        def get_secret_value(self, SecretId):
            return {"SecretString": "sc-live-key"}

    import boto3
    monkeypatch.setattr(boto3, "client", lambda *_a, **_k: _SM())
    import lookup_coa as lk
    importlib.reload(lk)
    assert lk.API_KEY == "sc-live-key"
    ...
    monkeypatch.setattr(boto3, "client", lambda *_a, **_k: _SMDenied())
    importlib.reload(lk)
    assert lk.API_KEY == "DEMO_KEY"   # reference data: public fallback is honest + rate-limited
