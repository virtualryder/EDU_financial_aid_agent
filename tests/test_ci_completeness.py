...
right behaviour on a laptop without the CDK libs installed - but CI never installed `aws-cdk-lib` at all, so for a period the assertions SKIPPED on every push, so the 24 CDK security
...
exact-ARN least-privilege IAM, and the "no student PII in Step Functions state" pass-by-reference
...
def test_cdk_libs_are_installed_in_ci():
    ...
    try:
        ...
        pytest.fail(
            ...
            "the 24 CDK security assertions would not run. Install cdk/requirements.txt in the "
