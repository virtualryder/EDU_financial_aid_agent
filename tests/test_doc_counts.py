...
Why this exists. Before this gate, four different counts (137, 150, 152, 153) coexisted across the
count-bearing documents in this repo - RELEASE-MANIFEST.md contradicted itself five rows apart
("150 / 150" in the summary table, "153 offline tests" in the prose below it), inside the file that
declares itself the authoritative count matrix. The cause was structural: `test_release_consistency.py`
gated tag references but nothing asserted a count, so every suite change silently invalidated the docs.

A reviewer who spot-checks one number and finds it wrong stops trusting the control claims too. For a
Title IV / FERPA audience, where the whole pitch is that the controls are verifiable, that is
expensive. So the count is now machine-enforced.
"""
import pathlib
import re
...
COUNTED_DOCS = [
    ...
    "PILOT-SCOPE.md", "EDU-PILOT-READINESS-PLAN.md", "EDU-PRODUCTION-PLAN.md",
    "DEPLOYMENT-GUIDE.md", "docs/GATE-B-CHECKLIST.md", "docs/MCP-GATEWAY.md",
    "evidence/EP1-VALIDATION.md",
]
...
def _collected_count():
    ...
    env = dict(os.environ, FA_DOC_COUNT_GATE="1")
    out = subprocess.run(
...
@pytest.mark.skipif(__import__("os").environ.get("FA_DOC_COUNT_GATE") == "1",
