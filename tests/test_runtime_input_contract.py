...
def test_plain_prompt_and_ids_accepted():
    prompt, case_id, requester = agent.validate_input({"prompt": "  Screen case FA-1  ", "case_id": "FA-1", "requester": "alice@agency"})
    assert prompt == "Screen case FA-1" and case_id == "FA-1" and requester == "alice@agency"
...
def test_runtime_model_guardrail_wiring(monkeypatch):
    ...
    class _Mcp:
        ...
        def call_tool_sync(self, *a, **k):   # P0-3 token_boundary wraps this on the EDU runtime
            return {}
