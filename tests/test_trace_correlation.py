...
settle. The audit line was never missing - it sat in /aws/lambda/edu-fp-write-audit at 11:02:37.666,
...
EXEC = "arn:aws:states:us-east-1:1234:execution:edu-fp-determination-workflow:lineage-lin-e02dbe"
...
def _read(messages, keys, case_id=CASE):
    logs = _Logs(messages)
    rows = tc.read_lambda_calls(logs, ["/aws/lambda/edu-fp-write-audit"], case_id, keys, *WINDOW)
...
def test_the_server_is_asked_only_for_a_window():
    ...
    assert scan["logGroupName"] == "/aws/lambda/edu-fp-write-audit"
