/* Leadership deck — Governed Agentic AI for Regulated Industries (Financial Aid accelerator on AgentCore) */
const pptxgen = require("pptxgenjs");
const p = new pptxgen();
p.layout = "LAYOUT_WIDE";
p.author = "Financial Aid AgentCore Accelerator";
p.title = "Governed Agentic AI for Federal Student Aid";
...
(() => {
  ...
  s.addText("AMAZON BEDROCK AGENTCORE  ·  HIGHER EDUCATION", { x: 0.7, y: 1.35, w: 11, h: 0.4, fontFace: BF, fontSize: 13, bold: true, color: MINT, charSpacing: 2, margin: 0 });
  ...
  const chips = ["31 / 31 live governance checks", "Agent native on Runtime", "Third vertical from one template"];
  let cx = 0.7;
  chips.forEach((c) => {
  ...
  s.addNotes("Third proof of the pattern: the same governed chassis that runs pharmacovigilance and benefits eligibility now runs Title IV financial-aid awarding, from one reusable template. AWS shipped the governance primitives; we implement the regulated pattern natively and prove it end-to-end.");
})();
...
(() => {
  ...
  title(s, "Aid offices want agentic AI for awarding. They can't deploy an ungoverned one.");
  s.addText([
    { text: "An agent that touches student PII and education records and issues aid determinations must be identity-bound, deny-by-default, auditable, and ", options: { color: INK, fontSize: 15, fontFace: BF } },
    { text: "incapable of awarding on its own.", options: { color: TEAL, fontSize: 15, fontFace: BF, bold: true } },
    { text: "  “Impressive demo” is not the bar — “will survive a program review and an appeal” is.", options: { color: INK, fontSize: 15, fontFace: BF } },
  ], { x: 0.62, y: 1.5, w: 12.0, h: 0.9, valign: "top", lineSpacingMultiple: 1.15, margin: 0 });
  const items = [
    ["Touches education records", "Names, SSNs, and tax-derived income flow through the FAFSA/ISIR.", "FERPA · GLBA · de-identification", TEAL],
    ["Creates records with consequences", "The award notice affects a student's aid and their right to appeal.", "Title IV · student appeal rights", AMBER],
    ["Takes consequential actions", "Committing an award grants or denies federal aid — accountably.", "Program integrity · separation of duties", MINT],
  ];
  ...
  footer(s, 2);
  s.addNotes("Three non-negotiables that block ungoverned agents in financial aid. The rest of the deck shows how AgentCore + three last-mile controls satisfy all three.");
})();
...
(() => {
  ...
  const prims = [
    ["Identity", "Inbound JWT — Cognito or the institution's IdP"],
...
(() => {
  ...
  const nativeList = ["Verified human + agent identity", "Deny-by-default authorization (Cedar)", "Least-privilege intersection (agent ∩ aid officer)", "Tools as governed endpoints (Gateway)", "Serverless agent runtime", "Tracing & observability"];
  ...
  const buildList = [["Fail-closed PII de-identification", "No masking → no assessment, no draft. Ever."], ["Human sign-off gate (separation of duties)", "A different qualified aid officer, single-use token."], ["Immutable WORM audit", "Append-only + Object Lock; writer can't tamper."]];
...
(() => {
  ...
  title(s, "Financial aid: awarding Pell & Title IV aid from a FAFSA");
  const steps = ["Intake the\nFAFSA / ISIR", "De-identify\nPII", "Assess Pell,\nSAP & verification", "Draft the\naward notice", "Aid-officer\nsign-off", "Commit the\naward"];
  ...
  s.addText([
    { text: "Under Title IV and program regulations, a qualified financial-aid officer makes and commits the award determination — applying professional judgment, SAP appeals, and verification. ", options: { bold: true, color: INK } },
  ...
  footer(s, 5);
  s.addNotes("Amber = the human step; navy = the consequential commit. The callout is the crux: an aid officer commits, the agent never self-awards.");
})();
...
(() => {
  ...
  const rows = [
    ["Verified human + agent identity", "AgentCore Identity — inbound JWT (Cognito / institution IdP)", true],
    ...
    ["Immutable WORM audit", "Append-only DynamoDB + S3 Object Lock — program-integrity evidence", false],
  ];
  ...
  footer(s, 6);
  s.addNotes("The full nine-control map — identical structure to the PV and benefits agents. That's the reusable pattern.");
})();
...
(() => {
  ...
  const flow = [
    ["1", "Aid officer authenticates", "Cognito / IdP issues a JWT for the person on whose behalf the agent acts."],
    ...
    ["6", "Human gate for the commit", "request_signoff opens a Step Functions gate; a second aid officer approves. Only then does finalize_award run."],
  ...
  footer(s, 7);
  s.addNotes("Steps 5 (fail-closed masking) and 6 (human gate) are the regulated controls; step 4 (Cedar deny-by-default) is the native spine.");
})();
...
(() => {
  const s = p.addSlide(); bg(s, CLOUD);
  eyebrow(s, "Evidence");
  title(s, "Proof, not slideware: 31 / 31 governance checks, live on AWS");
  const stats = [["31/31", "governance checks pass", MINT], ["6", "controls native on AgentCore", TEAL], ["3", "regulated controls built & proven", AMBER], ["0", "residual on teardown", NAVY]];
  ...
  const proofs = [
    "Deny-by-default — aid officer ALLOW, outsider DENY",
    "Mask-before-assess — un-masked assessment forbidden",
    "Mask-before-draft — un-masked notice forbidden",
    "No-self-commit — the agent can't finalize an award",
    "Real PII masking (Comprehend, fail-closed): name + SSN",
    "Deterministic Pell + SAP + verification engine",
    "Real Bedrock award notice through a Guardrail",
  ...
  footer(s, 8);
  s.addNotes("A reproducible, one-command demo stands the stack up, proves 31 checks in ENFORCE, and tears down with zero residual. The Runtime invoke (aid officer full workflow, outsider denied) was also proven live.");
})();
...
(() => {
  ...
  title(s, "The agent runs on AgentCore Runtime — driven by the aid officer's identity");
  s.addText("The aid officer authenticates with Cognito; that same identity flows to the Gateway, so Cedar evaluates the real person. The agent inherits the aid officer's least-privilege — it can do nothing they aren't allowed to do.",
  ...
  s.addText("Aid-officer identity", { x: 1.8, y: 2.85, w: 4.6, h: 0.5, fontFace: TF, fontSize: 17, bold: true, color: NAVY, valign: "middle", margin: 0 });
  s.addText("Runs the full governed workflow: intake → mask PII → assess aid → draft notice → write audit → request sign-off. The forbidden finalize tool is hidden from the agent entirely by Cedar.",
...
(() => {
  ...
  const cards = [
    ["Fail-closed PII de-identification", "Comprehend de-identifies the application before the model or the audit sees it. If masking can't run, assessment and drafting are blocked — no un-masked PII is ever emitted.", "FERPA · GLBA", TEAL],
    ["Human sign-off gate", "Committing an award is never an inline tool call. A Step Functions gate pauses for a different qualified aid officer, who approves with a bound, single-use token.", "Title IV · separation of duties", AMBER],
    ["Immutable WORM audit", "Append-only DynamoDB + S3 Object Lock capture INTENT → COMMITTED. The principal that writes evidence is denied delete, update, and bypass.", "Program-review evidence", MINT],
  ];
  ...
  footer(s, 10);
  s.addNotes("These three are where a governed agent earns a program reviewer's — and an appeal officer's — trust. Each maps to a named requirement.");
})();
...
(() => {
  ...
  const items = [
    ["Wins the higher-ed bar", "Colleges and universities carry strict FERPA, GLBA, and Title IV program-integrity requirements — native governance is the deciding factor, and AWS already holds the compliance footprint.", TEAL],
    ["A repeatable pattern, proven three times", "The same governed chassis produced the pharmacovigilance, benefits, and financial-aid agents from one template — swap domain logic and fixtures.", MINT],
    ...
    ["Honest by construction", "It's clear about what's proven vs. engagement work — exactly what institutions and their auditors trust.", NAVY],
  ];
  ...
  footer(s, 11);
  s.addNotes("Ties the technical work to AWS strategy: wins regulated higher-ed deals, reusable (now proven three times), low-risk, credible because honest.");
})();
...
(() => {
  ...
  ["The agent + the Cedar policies", "The six tools + fail-closed masking", "The deterministic Pell + SAP rules engine", "The human-gate workflow + WORM audit", "Reproducible IaC + one-command demo", "The documentation set"].forEach((t, i) => {
    const y = 2.45 + i * 0.53;
  ...
  s.addText("THE INSTITUTION OWNS  ·  ENGAGEMENT WORK", { x: 7.11, y: 1.85, w: 5.4, h: 0.4, fontFace: BF, fontSize: 12.5, bold: true, color: AMBER, charSpacing: 1, margin: 0 });
  ["IdP federation + aid-officer role mapping", "Connectors to the SIS / Common Origination & Disbursement", "Authoritative award rules & thresholds (compliance review)", "Notice language, SAP appeal rights, appeal process", "Computer-system validation + authorization to operate"].forEach((t, i) => {
    const y = 2.45 + i * 0.62;
  ...
  s.addText("The Pell/SAP figures shipped here are illustrative federal defaults — not authoritative rules. Nothing is production-certified on day one, and saying so is the credibility.", { x: 0.62, y: 5.95, w: 12.1, h: 0.7, align: "center", valign: "middle", fontFace: BF, fontSize: 13, bold: true, italic: true, color: WHITE, margin: 0 });
  footer(s, 12);
  s.addNotes("We own the pattern and the proof; the institution owns certification, connectors, and the authoritative award rules.");
})();
...
(() => {
  ...
  s.addText("One governed pattern, three verticals proven", { x: 0.6, y: 0.45, w: 12, h: 0.85, fontFace: TF, fontSize: 26, bold: true, color: WHITE, margin: 0 });
  s.addText("Three heroes now run on the same governed chassis — identity, Cedar, Gateway, Runtime, and the three last-mile controls — produced from one reusable template. Each new vertical is a domain-logic swap away.",
    { x: 0.62, y: 1.4, w: 11.6, h: 0.8, fontFace: BF, fontSize: 14, color: ICE, lineSpacingMultiple: 1.14, margin: 0 });
  const verts = [["HCLS", "Pharmacovigilance", "PROVEN", MINT], ["SLG", "Benefits Eligibility", "PROVEN", MINT], ["EDU", "Financial Aid", "PROVEN", MINT], ["HPP", "Healthcare Payer / Provider", "NEXT", TEAL]];
  ...
  const asks = [["1", "Internal demo", "Stand up the stack live and walk leadership through the 31-check proof."], ["2", "Institution workshops", "Take the pattern to higher-ed accounts as a reference architecture."], ["3", "Scoped pilot", "A pilot on synthetic FAFSAs with a named institution, boundary made explicit."]];
  asks.forEach((a, i) => {
    const x = 0.62 + i * 4.06;
  ...
  s.addNotes("Close on the multiplier: one pattern, three verticals proven, one to go. The ask is a three-step path from internal demo to a scoped, synthetic-data pilot.");
})();

p.writeFile({ fileName: "Financial-Aid-AgentCore-Leadership.pptx" }).then((f) => console.log("wrote", f));
