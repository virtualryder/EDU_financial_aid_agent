...
(() => {
  ...
  const stats = [["337/337", "automated tests green in CI", MINT], ["End-to-end", "proven live - inside a locked-down private network", TEAL], ["100%", "of determinations gated by an aid officer", AMBER]];
...
p.writeFile({ fileName: require("path").join(__dirname, "..", "Financial-Aid-AgentCore-Customer.pptx") }).then((f) => console.log("wrote", f));
