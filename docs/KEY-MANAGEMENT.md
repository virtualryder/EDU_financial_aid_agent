...
|---|---|---|---|---|
| `fa-<env>/provenance-signing-deid` | Signs mask_pii **sanitized-artifact refs** (proof of masking) | Secrets Manager (CMK-encrypted under `kms=customer-managed`) | `mask-pii` | assess, recertify, overpayment, core, guards |
| `fa-<env>/provenance-signing-scorecard` | Signs **College Scorecard reference-data provenance** (proof of authoritative source) | Secrets Manager (same) | `lookup-coa` | assess, guards |
| `fa-<env>/scorecard-api-key` | api.data.gov key (external credential, not a signing key) | Secrets Manager | — | `lookup-coa` only |
| `alias/fa-<env>-data` CMK | Encryption at rest: tables, WORM vault, secrets, Lambda env, log groups, SNS | KMS, **rotation enabled** | — | service principals + granted function roles only |

**Domain separation is enforced by IAM, not convention** (GA-2): the lookup role cannot read the
deid key and the masker cannot read the Scorecard key — CDK-asserted
...
**Environment separation:** keys are generated per deployment (`fa-<env>/…`) — dev, pilot, and any
future prod never share key material. **Tenant separation:** one institution per isolated deployment, so
...
2. **Rotate the secret value** (new version, same ARN):
   `aws secretsmanager put-secret-value --secret-id fa-<env>/provenance-signing-deid --secret-string "$(openssl rand -hex 48)"`
...
3. **Force consumers to re-read:** values are cached for the Lambda lifetime; publish a no-op env
   ...
   (`aws lambda update-function-configuration --function-name fa-<env>-mask-pii --environment ...`),
...
|---|---|---|---|
| deid key | Mint refs that verify as "masked" for THIS deployment/tenant | Forge Scorecard reference provenance; cross into another deployment/tenant (different key + tenant check); alter the WORM ledger; bypass the human gate | Rotate deid key (step 3); prior artifacts auditable by `key_version` |
| Scorecard key | Mint "verified" reference-COA figures for THIS deployment (estimates only - awards still require institutional COA + human approval) | Forge proof-of-masking; commit a determination (human gate + Cedar `no_self_commit` stand) | Rotate Scorecard key; cross-check suspect determinations against api.data.gov |
