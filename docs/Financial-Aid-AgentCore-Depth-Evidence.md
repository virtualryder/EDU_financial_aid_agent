# Financial Aid Agent — Depth Evidence (runtime trace + OAuth connector)

*Captured live against the deployed agent (us-east-1) with Cedar in **ENFORCE**, 2026-07. Account id is the live deploy; the public repo scrubs it. Together with the red-team harness (`bash lib/engine/redteam.sh agents/financial-aid`, governance-holds-under-attack), this is the portfolio depth pack applied to this vertical.*

---

## Item 1 — End-to-end agent runtime trace (Observability / X-Ray)

The Strands agent runs on **AgentCore Runtime** (`financial_aid_runtime_agent`). A aid_officer authenticates and their access token is the bearer for every governed Gateway (MCP) tool call, so Cedar evaluates the real human principal. With Transaction Search enabled, the per-invocation OTel spans are captured as one X-Ray trace. The captured trace (`1-6a5b0489-60505cb000c2a0c85d4bca58`, 54.6s) shows the agent autonomously orchestrating each **governed tool through the MCP gateway** and stopping at the human gate:

```
invoke_agent (Strands Agents)                          54.6s
├─ execute_tool  intake_fafsa                               495ms
├─ execute_tool  lookup_coa (live College Scorecard)        1609ms
├─ execute_tool  mask_pii                                   3084ms
├─ execute_tool  assess_aid                                 400ms
├─ execute_tool  draft_award_notice (guarded Bedrock)       9956ms
├─ execute_tool  write_audit (WORM)                         4236ms
└─ execute_tool  request_signoff (human gate)               3877ms
```

Every `execute_tool` span is a Cedar-authorized call through the governed gateway — including the **live authoritative-data lookup** and the **fail-closed PHI/PII masking** before the model ever drafts. The consequential `finalize_award` never appears: the agent completes everything it is allowed to do and then **waits on a human** (the sign-off gate is left in PENDING_APPROVAL). An outsider invocation returns `ACCESS DENIED, tools_available: []`.

## Item 2 — Real OAuth connector via AgentCore Identity outbound auth

`verify_source` calls a genuinely **OAuth2-protected** external system of record (`MOCK-SIS-COD`, an API-Gateway HTTP API that requires a Cognito M2M access token). The outbound token is minted by an **AgentCore Identity** OAuth2 credential provider (client_credentials / M2M) — the tool holds **no secret** (it lives in the Identity token vault). Proven live (`bash lib/connector/prove_connector.sh agents/financial-aid`):

```
1. the system of record REALLY requires OAuth  -> no-token 401, bad-token 401  (genuinely OAuth-protected)
2. governed tool: aid_officer calls verify_source; AgentCore Identity mints the outbound token -> verified
