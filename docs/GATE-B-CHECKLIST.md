...
|---|---|---|---|---|
| B1 | **Private networking + locked egress** — governed Lambdas in isolated subnets; ALL egress through AWS Network Firewall with a deny-by-default allowlist naming ONLY `.api.data.gov`; S3/DDB gateway + 7 interface endpoints keep AWS traffic on the AWS network; 443-only security group | `cdk/fa_stacks/network_stack.py` (`-c network_mode=private`) | CDK assertions (ported from the Housing pattern, live-proven there); **EDU live capture = EP1 validation run (pending)** | ✅ built + assertion-proven; EP1 live capture pending |
| B2 | **Customer-managed KMS** — one CMK (rotation on) over DDB tables, WORM vault, both signing secrets + Scorecard key, Lambda env vars, per-function log groups, SNS ops topic | `-c kms=customer-managed` (data/compute/observability stacks) | CDK assertions (ported from the Housing pattern, live-proven there); **EDU live capture = EP1 validation run (pending)** | ✅ built + assertion-proven; EP1 live capture pending |
| B3 | **Enterprise IdP + MFA** — `identity_mode=pilot`: MFA REQUIRED (software token only, SMS disabled), Cognito threat protection ENFORCED, admin-create-only, zero IaC users; OIDC IdP attachable as IaC (client secret via Secrets Manager dynamic reference — never plaintext) | `cdk/fa_stacks/identity_stack.py` + `docs/IdP-Federation-Reference.md` | CDK assertions (ported from the Housing pattern, live-proven there); **EDU live capture = EP1 validation run (pending)** (real-IdP round-trip = customer engagement work) | ✅ built + assertion-proven; EP1 live capture pending |
| B4 | **PII telemetry-leak canary** — unique fake-PII marker run through the live pipeline, then swept across CloudWatch Logs, X-Ray, Step Functions history, DLQs; `--strict` = every destination clean | `scripts/pii_canary.py` + pass-by-reference orchestration (`ingest-case` → encrypted case store → opaque refs only through the controller) | offline verdict tests + **LIVE STRICT PASS (2026-07-24): 0 hits in EVERY destination incl. SFN history** — the 87-hit finding remediated; CDK assertion pins zero-content state | ✅ built + assertion-proven; EP1 strict canary pending |
| B5 | **Tenant isolation** — tenant is deployment-pinned (`-c tenant=<pha-id>` → `TENANT_ID`), NEVER read from a request body; the pinned tenant is HMAC-signed into every `sanitized_ref`; verifiers refuse cross-tenant refs even with a valid signature. One institution per isolated deployment — no SaaS multi-tenancy claims | `lib/controls/tenancy.py`, `sanitized.py`, `mask_pii.py` | unit tests + CDK assertion + **LIVE: `tenant: pha-la-county` captured inside the signed artifact of the live run** | ✅ built + assertion-proven; EP1 live capture pending |
| B6 | **Load / concurrency / replay** — N concurrent executions must all reach legal terminal states; a concurrent replay storm against one approval must commit EXACTLY once (live confirmation of the GA-5 design) | `scripts/load_replay_test.py` | offline verdict tests + **LIVE: 10/10 concurrent SUCCEEDED; replay storm `FIRST:1, IDEMPOTENT:9`; 12 FINAL markers, one per case** | ✅ built + assertion-proven; EP1 live capture pending |

## Gate-B validation run — ☐ EP1 (pending; the Housing pattern this ports from captured it at
https://github.com/virtualryder/Housing_eligibility_agent — evidence/GATE-B-VALIDATION.md)
...
## Customer-owned governance (the accelerator provides the template; the institution signs)
...
the sanitized_ref control); retention per the selected profile; access (aid officers via the
IdP, MFA-required); disclosure paths (none outside the account); applicant rights + the
informal-hearing process. Owner: institution privacy officer. The threat model (`docs/THREAT-MODEL.md`) and
data-source policy feed sections 2–4.

**Retention approval.** The institution formally selects and signs the Object Lock retention profile
(`docs/RETENTION-PROFILES.md`) against its records schedule — never assert 7y as universal. Owner:
institution records officer.
...
customer-owned: governance signatures above, institution administrative-plan / notice-accessibility reviews,
