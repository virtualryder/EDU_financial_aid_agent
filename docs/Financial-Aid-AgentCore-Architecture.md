...
| Append-only, tamper-evident WORM audit (Title IV / program-integrity evidence) | Build | Append-only DynamoDB + S3 Object Lock; Observability traces are for ops, not tamper-proof evidence |
...
- **Append-only, tamper-evident WORM audit:** an append-only, tamper-evident record (append-only DynamoDB + S3 Object Lock) capturing `INTENT → COMMITTED` for the Title IV program-integrity and audit trail.
...
6. **Manifest + validate** — the whole agent is one manifest; deploy; end-to-end run (Cedar allow/deny, masking, aid determination, real Bedrock notice, tamper-evident audit) + negative tests; teardown.
...
- **FERPA** (protection of education records) → fail-closed masking + least-privilege Cedar + tamper-evident audit.
