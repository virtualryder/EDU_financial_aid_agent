...
| **Offline test suite** | **226 / 221** passing on current main (unit + eval + Cedar policy + **40 CDK stack-synthesis** assertions + the doc-count gate). 225 run locally; 1 CI-completeness gate runs only in CI. |
...
- **226 offline tests** — the CI suite (grew 132 at EP0 → 137 after doc-integrity → 150 → 153 after the
  Gate-B operating-model bundle added config/award-year/readability gates → 157 after the `fa-val2`
  re-validation gates → 175 → 190 → 205 → 226 today, including the doc-count gate). This is the current
