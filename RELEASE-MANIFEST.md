...
| **Offline test suite** | **337 / 337** passing on current main (unit + eval + Cedar policy + **44 CDK stack-synthesis** assertions + the doc-count gate). Every one runs locally except 1 CI-completeness gate, which runs only in CI. |
...
- **337 offline tests** — the CI suite (grew 132 at EP0 → 137 after doc-integrity → 150 → 153 after the
  Gate-B operating-model bundle added config/award-year/readability gates → 157 after the `fa-val2`
  re-validation gates → 175 → 190 → 205 → 271 today, including the doc-count gate). This is the current
