...
| **Offline test suite** | **301 / 301** passing on current main (unit + eval + Cedar policy + **44 CDK stack-synthesis** assertions + the doc-count gate). Every one runs locally except 1 CI-completeness gate, which runs only in CI. |
...
- **301 offline tests** — the CI suite (grew 132 at EP0 → 137 after doc-integrity → 150 → 153 after the
