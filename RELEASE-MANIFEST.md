...
| **Offline test suite** | **271 / 221** passing on current main (unit + eval + Cedar policy + **44 CDK stack-synthesis** assertions + the doc-count gate). 264 run locally; 1 CI-completeness gate runs only in CI. |
...
- **298 offline tests** — the CI suite (grew 132 at EP0 → 137 after doc-integrity → 150 → 153 after the
