...
_DOMAINS = {
    ...
    "scorecard": ("PROVENANCE_SECRET_SCORECARD", "PROVENANCE_SECRET_ARN_SCORECARD"),
}
...
def _secret(domain=None):
    """Resolve the signing secret for a trust domain (GA-2). Resolution order:
      1. domain-scoped env / Secrets Manager ARN (PROVENANCE_SECRET[_ARN]_{DEID|SCORECARD}) — production;
...
def sign(source, fields, domain=None):
    ...
    key of the caller's trust `domain` (GA-2: 'scorecard' for the reference-data lookup, 'deid' for the
