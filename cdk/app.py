...
from fa_stacks.identity_stack import IdentityStack
...
def stage_lambda_bundle():
    ...
    control plane silently diverged while every integrity lock recorded the same tree hash. For an
    aid workflow the specific consequence of a missing exactly-once finalization control is a
    duplicate award or adjustment posted for the same student. A copy can silently diverge; a
    ...
    Layering is deliberate and ordered:
      ...
      2. lib/controls                  — this agent's domain-shaped modules (mask_pii, provenance,
                                         ...
                                         ingest_case, tenancy, readability)
    ...
    for src in (str(governed_core.controls_dir()),
                ...
                os.path.join(REPO, "agents", "financial-aid", "tools")):
