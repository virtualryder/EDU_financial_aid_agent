    ...
    def __init__(self, scope: Construct, cid: str, *, prefix: str,
        ...
        if fed.get("issuer_url") and fed.get("client_id"):
            ...
            self.pool.add_domain("FederationDomain", cognito_domain=cognito.CognitoDomainOptions(
                domain_prefix=f"{prefix}-finaid".replace("_", "-")))
        ...
        # gateway interceptor both read it. An aid officer holds aid_officer + exactly one tenant group.
        self.tenant_groups = {}
        for t in tenants:
        ...
        cognito.CfnUserPoolGroup(self, "ReviewerGroup", user_pool_id=self.pool.user_pool_id,
                                 group_name="aid_officer",
                                 description="Qualified financial-aid officers (Cedar role group)")
        ...
        cdk.CfnOutput(self, "FederationNote",
                      value=("Enterprise OIDC IdP attached as IaC; map IdP groups to aid_officer "
