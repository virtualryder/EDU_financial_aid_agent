...
external destination is the College Scorecard API (api.data.gov) (the pipeline's single sanctioned external dependency).
...
class NetworkStack(cdk.Stack):
    def __init__(self, scope: Construct, cid: str, *, prefix: str, **kw):
        ...
        # ── deny-by-default egress: allowlist ONLY the HUD API ───────────────
        rule_group = nfw.CfnRuleGroup(
            ...
            description="Gate-B locked egress: College Scorecard API is the ONLY sanctioned external destination")
