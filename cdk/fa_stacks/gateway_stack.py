...
_PERIMETER_INPUT_FIELDS = {
    "consent": {"type": "boolean", "description": "Perimeter gate: recorded student consent for this action (coarse gateway gate; the authoritative consent artifact is server-side)."},
    "purpose": {"type": "string", "description": "Perimeter gate: declared processing purpose (purpose-limitation), e.g. aid_determination / verification."},
...
def _targets_from_manifest(compute, perimeter=False):
    ...
    m = yaml.safe_load((REPO / "agents" / "financial-aid" / "manifest.yaml").read_text(encoding="utf-8"))
    fn_by_target = {
        "intake-fafsa": compute.intake, "lookup-coa": compute.lookup,
        "mask-pii": compute.mask, "assess-aid": compute.assess,
        "verify-docs": compute.verify_docs, "record-pj": compute.pj,
        "fa-core": compute.core, "write-audit": compute.write_audit,
    ...
    for t in m["tools"]:
        ...
        for tool in t["mcp_tools"]:
            ...
            # the schema ONLY so the gateway maps them through; the target trusts them ONLY if the
            # signature verifies (tenancy.verified_tenant_from_args). Caller values are overwritten.
...
class GatewayStack(cdk.Stack):
    def __init__(self, scope: Construct, cid: str, *, prefix: str, compute, identity,
        ...
        ssm_param = f"/{prefix}-aid/gateway-url"
        ...
        attachment = cdk.CustomResource(
            ...
            properties={
                "EngineName": f"{prefix.replace(chr(45), chr(95))}_aid_authz",   # API: ^[A-Za-z][A-Za-z0-9_]*$ (live-run find)
                "EngineDesc": "Deny-by-default Cedar authz (IaC-attached)",
                "GatewayName": f"{prefix}-aid-gw",
                "GatewayDesc": "Financial-aid governed tool gateway (IaC-attached)",
