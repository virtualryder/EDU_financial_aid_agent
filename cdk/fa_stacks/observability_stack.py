...
the pager seam (subscribe email/PagerDuty at deploy). governed-core 1.7-1.9 additions: Bedrock
model-invocation logging + the AgentCore gateway's vended request logs (full transparency), per-tenant
budget alarms, and the AWS Budgets USD ceiling whose breach engages the kill switch."""
...
class ObservabilityStack(cdk.Stack):
    def __init__(self, scope: Construct, cid: str, *, prefix: str, compute, workflow,
        ...
        # A nonzero sum means forged/tampered/missing evidence (a forged sanitized_ref, tampered
        # Scorecard provenance, a spoofed boolean, or a source-down lookup) hit a guard — page immediately.
        ...
        alarm("GuardFailures", guard_failed,
              desc="A workflow guard REFUSED evidence (forged sanitized_ref, tampered Scorecard "
                   "provenance, spoofed boolean, or source-down). Security signal - triage per "
                   "THREAT-MODEL.md; repeated failures may indicate an active forgery attempt.")
        ...
        dash.add_widgets(
            ...
            cw.GraphWidget(title="Scorecard lookup: invocations vs errors (source availability)", width=12,
    ...
    def _transparency(self, prefix, gateway, model_logging):
        ...
        logs are AgentCore-managed. Sources: Bedrock model-invocation logging + AgentCore observability."""
