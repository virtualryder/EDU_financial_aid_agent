...
| `fa-<env>-gateway` | **the full AgentCore attachment AS IaC** (GA-1, live-validated twice): custom-resource provider creates the Cedar policy engine → MCP gateway (CUSTOM_JWT via the identity pool) → SSM discovery param → one target per governed tool (exact ARNs, schemas synthesized from the manifest) → all Cedar policies → **ENFORCE**; stack delete reverses everything | GA-1 · GA-4 |
