"""Governed intake agent (EDU financial-aid; governed-core 1.9.0 runtime, benefits parity + P0-3 token boundary) — runs natively on Amazon Bedrock AgentCore Runtime.
...
import token_boundary  # P0-3 (EDU): trusted runtime credential boundary (bundled beside this file)
...
def _meta_value(v):
    r"""Converse.requestMetadata values: ≤256 chars from [a-zA-Z0-9\s:_@$#=/+,-.] (API reference)."""
...
def invoke(payload, context=None):
    ...
    # P0-3 (EDU): the runtime is the credential boundary. Model-supplied credential-shaped args are
    # scrubbed from EVERY governed tool call; the runtime-held access token is injected out-of-band
    # into the sign-off call only — the model never sees, produces, or transports a bearer token.
    token_boundary.wrap_mcp_client(mcp_client, token)
    # outcome is decided INSIDE the session; a teardown artifact must never overwrite it (see the outer
    # except). Every path sets `outcome`, and the function returns it exactly once.
    outcome = None
    try:
        with mcp_client:
            ...
            if not tools:
                ...
                # trace_attributes land on EVERY Strands span of this invocation (invoke_agent, cycles,
                # model invoke, execute_tool): session.id (mandatory for AgentCore observability) + tenant.
                agent = Agent(model=model, tools=tools, system_prompt=SYSTEM, trace_attributes=corr)
                try:
                    result = agent(prompt)
                    outcome = {"result": str(result), "tools_available": names, "tenant": session_tenant}
                except Exception as exc:              # stopped mid-session: report, never retry
                    # Strands wraps a hook exception in strands.types.exceptions.EventLoopException (seen
                    # live 2026-09-03: "Invocation failed ... EventLoopException, message = our reason"),
                    # so walk the cause chain and ALSO re-read the switch: either one proves containment.
                    b = _budget_stopped(exc)
                    if b is not None:
                        outcome = {**_budget_refusal(b.decision, corr), "tools_available": names,
                                   "tenant": session_tenant, "stopped": "mid-session"}
                    elif _contained(exc):
                        engaged = _kill_switch() or {"reason": str(exc), "source": ",".join(KILL_SWITCH_PARAMS)}
                        outcome = {**_refusal(engaged, corr), "tools_available": names,
                                   "tenant": session_tenant, "stopped": "mid-session"}
                    else:
                        raise
    except Exception as exit_exc:
        # The outbound MCP client's streamable-HTTP teardown can raise "Connection to the MCP server was
        # closed" while a kill-switch / budget stop unwinds the in-flight session (seen live 2026-09-04:
        # the refusal was masked by the close error -> a 500 instead of a governed mid-session stop). A
        # teardown artifact must NEVER mask a governance decision: if the in-session logic already decided
        ...
        if outcome is None:
            if _contained(exit_exc):
                ...
                raise
    rc = len(str(outcome.get("result", ""))) if isinstance(outcome, dict) else 0
    log.info("invocation_complete requester=%s case_id=%s result_chars=%d", requester, case_id, rc)
    return outcome
