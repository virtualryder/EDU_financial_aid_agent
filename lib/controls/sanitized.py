...
# sanitized.py — server-issued SANITIZED-ARTIFACT references (P0-1).
#
# THE DEFECT THIS FIXES: downstream tools (assess / recertify / overpayment / draft) authorized on a
# `deidentified: true` BOOLEAN that arrives in the tool call body — i.e., a claim the model/caller makes
# about masking, not proof that masking actually occurred. `{"case":"<unmasked PII>","deidentified":true}`
# satisfied both the Cedar forbid and the in-body check. A flag you can type is not a control.
#
# THE FIX (same pattern that fixed income-limit provenance, provenance.py): the ONLY component that
# actually performed the masking (mask_pii, which alone ran Comprehend DetectPiiEntities) MINTS a signed
# sanitized-artifact reference over the EXACT masked content:
#
#     sanitized_ref = { artifact_id, sanitized_sha256, engine, entities_masked, tenant, ts,
#                       source, authoritative, sig, alg }
#
...
#   * artifact store          -> CONTENT CHANNEL: when a store is configured (SANITIZED_TABLE, provisioned
#     by the CDK stack), the sanitized payload is persisted server-side at mint and retrieved server-side
#     by consumers, so sanitized content need not travel through the model at all. Without a store the
#     hash binding still guarantees integrity; the store additionally removes the content from model
...
SOURCE = "mask_pii — fail-closed PII de-identification (server-issued sanitized artifact)"
...
_SIGNED_FIELDS = ("artifact_id", "sanitized_sha256", "engine", "entities_masked", "tenant", "ts")
...
class DynamoStore:
    """DynamoDB-backed store (table from SANITIZED_TABLE; provisioned by the CDK Data stack with
    PutItem/GetItem least privilege and a TTL attribute)."""
    ...
    if st is not None:
        try:
            stored = bool(st.put(fields["artifact_id"], masked_text,
                                 {"sanitized_sha256": fields["sanitized_sha256"],
                                  "tenant": tenant, "engine": engine}))
        except Exception:
            stored = False
    ref["stored"] = stored
    return ref
