...
| Field | Value |
|---|---|
| Tag | `v0.4.0-pilot-rc1` — the supported pilot tag (reconciled docs + Gate-B operating-model bundle). Single source of truth: repo-root `RELEASE`, enforced by `tests/test_release_consistency.py`. EP1 was validated on the code first cut as `v0.1.0-pilot-rc1`; `v0.1.1` adds documentation + tests only (**no infrastructure change**), so the EP1 evidence carries forward. **`v0.4.0-pilot-rc1` (2026-09-05) supersedes `v0.3.0-pilot-rc1 / v0.1.3-pilot-rc1`**: the same tree with `lib/` relocked at the pinned **governed-core 1.10.1** (fault-semantics batch — durable-evidence gating, un-strandable approvals, authoritative Cedar context — plus deepened PII/PHI detection); **offline-gated** (all pass locally except one CI-only gate; 267 collected). The last LIVE gate on this pack ran on governed-core 1.9.0 (2026-09-04, from-zero two-tenant AgentCore deployment; tag `v0.3.0-pilot-rc1`); a live re-gate on 1.10.1 is the next milestone for this pack (third external review, REL-2). |
| Commit SHA | the commit carrying tag `v0.4.0-pilot-rc1` (`git rev-list -n1 v0.4.0-pilot-rc1`) |
| Test count | **267 tests** on current main (offline + CDK assertions + the doc-count gate). Authoritative count matrix: [`RELEASE-MANIFEST.md`](RELEASE-MANIFEST.md) (132 at EP0 → 137 → 150 → 153 → 157 → 175 → 190 → 205 → 267; do not cite older counts). Tag `v0.4.0-pilot-rc1` (2026-09-05, offline-gated on governed-core 1.10.1) was cut at 205 <!-- count-gate:historical --> tests; the 2026-09-06 PAR-1 port of the 2026-09-05 controls (capture-all lineage, enforcement perimeter, restore-aware model logging, pinned drafter role) is on main after that tag; `v0.3.0-pilot-rc1` (governed-core 1.9.0 live-gated 2026-09-04) precedes it; `v0.2.0-pilot-rc1` (offline governed-core migration) and `v0.1.3-pilot-rc1` sit earlier in that sequence. |
