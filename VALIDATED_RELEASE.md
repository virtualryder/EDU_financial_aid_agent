...
| Field | Value |
|---|---|
| Tag | `v0.1.1-pilot-rc1` — the supported pilot tag (reconciled docs + Gate-B operating-model bundle). Single source of truth: repo-root `RELEASE`, enforced by `tests/test_release_consistency.py`. EP1 was validated on the code first cut as `v0.1.0-pilot-rc1`; `v0.1.1` adds documentation + tests only (**no infrastructure change**), so the EP1 evidence carries forward. |
